...
边缘多了一个空格——都会采用去缩进形式 `( ... )`,或者当去缩进会
改变内容时采用逐字形式 `(( ... ))`;无论 writer 选择哪种形式,
round-trip 都是字节级无损的。

具体采用哪种块形式,取决于空白出现在**哪一侧**。末尾空格在去缩进
形式下可以保留,因为去缩进只移除共同的**前导**缩进:
...
一行,或所有行都有缩进(没有可用于将去缩进基准定为零的行),此时
