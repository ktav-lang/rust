...
pub(crate) struct InlineBounds<'a> {
    pub(in crate::document::parser::inline) origin: usize, // address of the top body's first byte (coordinate origin)
    pairs: &'a [(usize, usize)],
}
...
impl<'a> InlineBounds<'a> {
    ...
    pub(in crate::document::parser::inline) fn opener_close_at(&self, abs: usize) -> Option<usize> {
        #[cfg(test)]
        if ix_probe::bypass_engaged() {
            return None;
        }
...
pub(in crate::document::parser::inline) fn malformed(
    line_num: usize,
    span: Span,
    detail: &str,
) -> Error {
