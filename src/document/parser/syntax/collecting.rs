...
#[derive(Copy, Clone)]
pub(in crate::document::parser) enum MultilineMode {
...
impl MultilineMode {
    pub(in crate::document::parser) fn terminator(self) -> &'static str {
        match self {
...
pub(in crate::document::parser) struct Collecting<'a> {
    pub(in crate::document::parser) mode: MultilineMode,
    pub(in crate::document::parser) lines: Vec<&'a str>,
}

impl<'a> Collecting<'a> {
    pub(in crate::document::parser) fn new(mode: MultilineMode) -> Self {
        Self {
            mode,
    ...
    pub(in crate::document::parser) fn is_terminator(&self, trimmed: &str) -> bool {
        trimmed == self.mode.terminator()
    }

    pub(in crate::document::parser) fn finish(self) -> String {
        match self.mode {
            MultilineMode::Verbatim => {
