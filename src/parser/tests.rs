...
fn classify_inline_nonempty_rejected() {
    assert!(classify_value_start("{a: 1}", 1, S).is_err());
    assert!(classify_value_start("[1, 2]", 1, S).is_err());
}
...
fn typed_integer_accepts_plain_digits() {
    assert_eq!(validate_typed_integer(" 42 ", 1, S).unwrap(), "42");
    assert_eq!(validate_typed_integer("0", 1, S).unwrap(), "0");
    assert_eq!(
        validate_typed_integer("9999999999999999999999", 1, S).unwrap(),
...
fn typed_integer_strips_leading_plus() {
    assert_eq!(validate_typed_integer(" +5", 1, S).unwrap(), "5");
    assert_eq!(validate_typed_integer(" +0", 1, S).unwrap(), "0");
}
...
fn typed_integer_rejects_non_digit() {
    assert!(validate_typed_integer(" abc", 1, S).is_err());
    assert!(validate_typed_integer(" 1.5", 1, S).is_err());
    assert!(validate_typed_integer(" ", 1, S).is_err());
    assert!(validate_typed_integer("", 1, S).is_err());
    assert!(validate_typed_integer(" +", 1, S).is_err());
    assert!(validate_typed_integer(" -", 1, S).is_err());
}
...
fn typed_integer_error_mentions_category() {
    let err = validate_typed_integer(" abc", 5, S).unwrap_err();
    let msg = format!("{}", err);
...
fn typed_float_accepts_simple() {
    assert_eq!(validate_typed_float(" 3.14", 1, S).unwrap(), "3.14");
    assert_eq!(validate_typed_float(" 0.0", 1, S).unwrap(), "0.0");
}
...
fn typed_float_preserves_minus_and_exponent_signs() {
    assert_eq!(validate_typed_float(" -3.14", 1, S).unwrap(), "-3.14");
    assert_eq!(validate_typed_float(" 3.14e+10", 1, S).unwrap(), "3.14e+10");
    assert_eq!(validate_typed_float(" 3.14E-10", 1, S).unwrap(), "3.14E-10");
    assert_eq!(validate_typed_float(" 3.14e10", 1, S).unwrap(), "3.14e10");
}
...
fn typed_float_rejects_invalid_exponent() {
    assert!(validate_typed_float(" 1.5e", 1, S).is_err());
    assert!(validate_typed_float(" 1.5e+", 1, S).is_err());
    assert!(validate_typed_float(" 1.5ee5", 1, S).is_err());
}

#[test]
fn typed_float_rejects_empty_and_compound() {
    assert!(validate_typed_float("", 1, S).is_err());
    assert!(validate_typed_float(" ", 1, S).is_err());
    assert!(validate_typed_float(" {", 1, S).is_err());
    assert!(validate_typed_float(" [", 1, S).is_err());
    assert!(validate_typed_float(" (", 1, S).is_err());
