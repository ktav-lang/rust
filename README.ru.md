...
Замыкающий пробел ниже — как раз тот случай: в одной строке парсер его
бы срезал, поэтому writer берёт блочную форму.
...
Прочитав это обратно, получите `"hunter2 "` — пробел на месте.
