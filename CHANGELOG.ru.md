...
Фича `cabi` превращает шесть приватных копий C ABI-шима из шести
...
- **`ktav::cabi` и макрос `declare_cabi!`** — разделяемая половина
  ...
  кодирование любого сбоя — включая не-ktav-овские — в конверт. В модуле нет ни одного `extern "C"`:
